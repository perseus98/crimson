...
// }
function is_founder(){
    return is_login() && strcasecmp($_SESSION["role"],"Founder")==0;
}
...
function check_img(){
    if(strlen($_FILES["at1"]["name"])!=0){
        ...
        if($ext != ".jpg" || $ext != ".png"){
            return "Image not supported, only jpg or png is allowed!";
        }
        else if($_FILES["at1"]["size"]/(1024*1024) > 6){
            return "Image is too large, max 6 MB is allowed!";
        }       
    }
    return null;
}
